...
export const MeasureView: React.FC<MeasureViewProps> = ({ 
  ...
  const rotateCamera = async () => {
    try {
      // Logic is handled entirely in Python; we just trigger the command
  ...
  return (
    <div className="flex h-full w-full gap-4 p-4 pr-0 relative"> 
      ...
      <div className="flex-1 bg-black rounded-2xl relative overflow-hidden flex items-center justify-center border-2 border-cyber-gray shadow-2xl group">
        <div className="relative w-full h-full flex items-center justify-center overflow-hidden">
           {lastResult && !isRawMode ? (
             <img 
              src={lastResult.measure_image} 
              alt="Result" 
      ...
      <div className="flex flex-col gap-4 w-96 shrink-0 z-10 p-4 pl-0">
        ...
        <div className="flex gap-2">
          ...
          <button 
            onClick={toggleGarmentType}
            className="flex-1 bg-cyber-blue hover:bg-blue-600 text-white font-bold py-3 rounded-lg text-center text-sm uppercase transition-colors shadow-glow-blue"
          >
        ...
        <div className="flex-1 bg-cyber-dark rounded-3xl p-6 flex flex-col items-center justify-center text-center overflow-y-auto shadow-inner border border-cyber-gray relative">
          ...
          {lastResult ? (
            <div className="w-full h-full flex flex-col gap-2 relative z-10">
              ...
              <button onClick={handleSaveReport} className="mt-auto bg-gray-800 hover:bg-gray-700 text-white py-3 rounded-lg text-xs uppercase tracking-widest border border-gray-600 transition-colors">
                Save QC Report
              </button>
            </div>
          ) : (
        ...
        <div className="flex gap-4 h-24 mt-2">
          <button 
            onClick={() => handleLiveRaw()}
            className={`flex-1 rounded-3xl font-bold text-xl uppercase tracking-wider transition-all shadow-lg border border-transparent 
              ${isRawMode 
                ? 'bg-transparent border-cyber-cyan text-cyber-cyan shadow-glow-cyan' 
                : 'bg-cyber-gray text-gray-400 hover:text-white hover:bg-gray-700'}`}
          >
            Live View
          </button>
          ...
          <button 
            ...
            className="flex-1 bg-cyber-blue hover:bg-blue-600 rounded-3xl text-white font-bold text-xl uppercase tracking-wider shadow-glow-blue transition-transform active:scale-95"
          >
            Capture
          </button>
        </div>
